...
@section('content')
                                ...
                                <td>
                                    <div class="hstack gap-2 fs-15">
                                        ...
                                        <button class="btn btn-icon btn-sm btn-info editBtn" data-id="{{ $i->id }}"
                                            data-name="{{ $i->sub_category_name }}" data-slug="{{ $i->slug }}"
                                            data-description="{{ $i->description }}"><i class="ri-edit-line"></i></button>
                                            <a href="#" class="btn btn-icon btn-sm btn-danger" onclick="confirmDelete('{{ route('admin.sub_category.destroy', $i->id) }}')"><i class="ri-delete-bin-6-line"></i></a>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">Chỉnh sửa danh mục con</h5>
                ...
                <div class="modal-body">
                    <form id="editForm" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="edit_category_name" class="form-label">Tên Danh Mục Con:</label>
    ...
    <script>
        ...
        $('.editBtn').click(function() {
            ...
            var updateUrl = "{{ route('admin.sub_category.update', ['id' => ':id']) }}";
            updateUrl = updateUrl.replace(':id', id);

            $('#editForm').attr('action', updateUrl); // Điều chỉnh URL của form dựa vào ID
            $('#editForm input[name="sub_category_name"]').val(name);
